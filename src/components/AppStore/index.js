...
class AppStore extends Component {
  ...
  onChangeSearchItem = event => {
    ...
    this.setState({searchAppValue: searchText})
  }

  getActiveAppsList = () => {
    const {activeTabId, searchAppValue} = this.state

    let activeAppsList = appsList.filter(
      appItem => appItem.category === activeTabId,
    )

    if (searchAppValue !== '') {
      console.log(searchAppValue)
      activeAppsList = activeAppsList.filter(appItem =>
        appItem.appName.toLowerCase().includes(searchAppValue.toLowerCase()),
      )
    }
    return activeAppsList
  }
  ...
  render() {
    ...
    const mainEle = (
      <div className="mainBgContainers">
        <div className="bgContainer">
          <h1 className="headerText">App Store</h1>
          ...
          <ul className="tabItemsContainer">
            {tabsList.map(tabItem => (
              <TabItem
                ...
                onClickGetActiveTabId={this.onClickGetActiveTabId}
              />
            ))}
          </ul>

          <ul className="appBgContainer">
            {activeAppsList.length === 0 && (
              <h1 className="noItemsFound">No Items Found</h1>
            )}

            {activeAppsList.length !== 0 &&
              activeAppsList.map(appItem => (
